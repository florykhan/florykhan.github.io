...
> 🗒️ **Note:**  
> The `dist/` directory is **generated by `deploy-prepare.sh`** during CI and is **not tracked by Git**. Paths are rewritten so links work under GitHub Pages (`/personal-website/`). Local development uses `src/` directly — paths assume pages are under `src/pages/`.
...
**HTTPS (recommended):**
```bash
git clone https://github.com/florykhan/personal-website.git
cd personal-website
```

**SSH (if you have keys configured):**
```bash
git clone git@github.com:florykhan/personal-website.git
cd personal-website
```
...
- The `deploy-prepare` script copies `src/` into `dist/` and rewrites `../styles/`, `../assets/`, `../scripts/` so links work at `/<repo>/`.

---

## 📄 Project Detail Page

For a full write-up — Problem & Context, What It Does, Tech Stack, Architecture, Key Takeaways — see the project detail page on the live site:

**[Personal Website — Project Detail](https://florykhan.github.io/personal-website/projects/personal-website.html)**
